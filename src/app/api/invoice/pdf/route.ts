...
import { extractField, extractNumber, getInvoiceNumber } from '@/lib/invoice/utils';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const bookingDateRaw = extractField(body, ['tanggal', 'event_date', 'tanggal_kegiatan'], '');
    ...
    const invoiceData: InvoiceData = {
      ...
      payment_deadline: extractField(body, ['payment_deadline', 'batas_pembayaran']),
      payment_status: extractField(body, ['payment_status', 'status_pembayaran']),
      ...
      paket: extractField(body, ['paket', 'package_name', 'paket_photobooth'], 'Paket Basic 4 Jam'),
      duration: extractField(body, ['duration', 'durasi']),
      print_capacity: extractField(body, ['print_capacity', 'kapasitas_print']),
      
      harga: extractNumber(body, ['harga', 'unit_price', 'harga_satuan', 'total_price']),
    ...
    const stream = await renderToStream(React.createElement(InvoicePDF, { data: invoiceData }) as any);
